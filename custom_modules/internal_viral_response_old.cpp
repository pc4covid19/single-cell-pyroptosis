...
*/	
	...
	if( AV > 0)// pCell->custom_data["max_apoptosis_half_max"]*3/2 )
	{
...
void pyroptosis_cascade( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	//Update nuclear NFkB
	double dNFkB_n = dt_test * ( F_ib*k_nfkb_ctn*(1-pCell->custom_data[nfkb_n]) - k_nfkb_ntc*pCell->custom_data[nfkb_n]);
	pCell->custom_data[nfkb_n] += dNFkB_n; 
	printf("NFkB %lf \n",pCell->custom_data[nfkb_n]);
	...
	double dNLRP3_i = dt_test * ( a_nlrp3*hill_nfkb - (k_nlrp3_ita + d_nlrp3)*pCell->custom_data[nlrp3_i] ); 
	pCell->custom_data[nlrp3_i] += dNLRP3_i;

	double dNLRP3_a = dt_test * (k_nlrp3_ita*pCell->custom_data[nlrp3_i] - (k_nlrp3_atb*F_ib + d_nlrp3)*pCell->custom_data[nlrp3_a]);
	pCell->custom_data[nlrp3_a] += dNLRP3_a;

	double dNLRP3_b = dt_test * k_nlrp3_atb * F_ib * pCell->custom_data[nlrp3_a];
	pCell->custom_data[nlrp3_b] += dNLRP3_b; 

	//Update bound ASC
	double dASC_b = dt_test * k_asc_ftb * (1-F_ib) * pCell->custom_data[nlrp3_b]*(1-pCell->custom_data[asc_b]);
	pCell->custom_data[asc_b] += dASC_b; 

	//Update bound caspase1
	double dCasp1 = dt_test * k_c1_ftb * pCell->custom_data[asc_b]*(1-pCell->custom_data[caspase1_b]);
	...
	double dGSDMD_c = dt_test * a_gsdmd * hill_caspase1 * (1-pCell->custom_data[gsdmd_c]);
	...
	double dIL1b_p = dt_test * (a_il1b_p*hill_nfkb - a_il1b_c*hill_caspase1*pCell->custom_data[il_1b_p] - d_il*pCell->custom_data[il_1b_p]);
	pCell->custom_data[il_1b_p] += dIL1b_p;

	double dIL1b_c = dt_test * (a_il1b_c*hill_caspase1*pCell->custom_data[il_1b_p] - k_il1b_cte*g_gsdmd*pCell->custom_data[il_1b_c] - d_il*pCell->custom_data[il_1b_c]);
	pCell->custom_data[il_1b_c] += dIL1b_c;

	double dIL1b_e = dt_test * (k_il1b_cte*g_gsdmd*pCell->custom_data[il_1b_c]);
	pCell->custom_data[il_1b_e] += dIL1b_e; //We want to relate this to secreted cytokine IL1b

	//Update IL18 (cytoplasmic, external)
	double dIL18_c = dt_test * (a_il18*hill_caspase1*(1-pCell->custom_data[il_18_c]-pCell->custom_data[il_18_e]) - k_il18_cte*g_gsdmd*pCell->custom_data[il_18_c]  );
	pCell->custom_data[il_18_c] += dIL18_c;

	double dIL18_e = dt_test * k_il18_cte*g_gsdmd*pCell->custom_data[il_18_c];
	pCell->custom_data[il_18_e] += dIL18_e;//We want to relate this to secreted cytokine IL18

	//Update cytoplasmic volume
	double dVol_c = dt_test * k_vol_c * g_gsdmd * pCell->custom_data[volume_c];
